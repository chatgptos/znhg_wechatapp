// pages/crowdapply/order/order.js
...
Page({
  ...
  onShareAppMessage: function () {
    ...
    orderCancel:function(e)
    {
        ...
        app.request({
            url: api.crowdapply.order_cancel,
            data: {
                id: id,
            },
            success: function (res) {
                if (res.code == 0) {
                    wx.redirectTo({
                        url: '/pages/crowdapply/order/order?status=0'
                    })
                }
            },
            complete: function () {
        ...
        app.request({
            ...
            success: function (res) {
                ...
                if (res.code == 0) {
                    ...
                    wx.redirectTo({
                        url: "/pages/crowdapply/order/order?status=1",
                    });
    ...
    goToDetails:function(e){
        console.log(e);
        wx.navigateTo({
            url: '/pages/crowdapply/order/details?oid=' + e.currentTarget.dataset.id,
        });
    },
    /**
     * 核销码
    ...
    applyRefund:function(e)
    {
        ...
        app.request({
            url: api.crowdapply.apply_refund,
            data: {
                order_id: id
            },
            success: function (res) {
                if (res.code == 0) {
                    wx.showModal({
                        ...
                        success: function (res) {
                            if (res.confirm) {
                                wx.redirectTo({
                                    url: "/pages/crowdapply/order/order?status=3",
                                });
                            }
                        }
                    })
    ...
    comment:function(e){
        wx.navigateTo({
            url: '/pages/crowdapply/order-comment/order-comment?id='+e.target.dataset.id,
            success: function(res) {},
            fail: function(res) {},
